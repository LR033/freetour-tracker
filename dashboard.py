...
st.subheader("Average position by tour")

avg_options = ["Last 7 days", "Last 30 days", "Last 90 days", "All time"]
avg_window = st.selectbox("Period", avg_options, index=1)

avg_cutoffs = {"Last 7 days": 7, "Last 30 days": 30, "Last 90 days": 90}

if avg_window == "All time":
    avg_df = filtered.copy()
else:
    cutoff = end_date - timedelta(days=avg_cutoffs[avg_window])
...
avg_data["color"] = avg_data["tour"].apply(lambda t: colors[tours.index(t) % len(colors)] if t in tours else "#aaa")
...
avg_fig.update_layout(
    xaxis_title="Average position (lower = better)",
...
avg_fig.update_xaxes(tickfont_color="#ffffff", title_font_color="#ffffff", gridcolor="rgba(255,255,255,0.15)")
